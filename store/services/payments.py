...
def sign_bepaid_payload(params: dict) -> str:
    """
...
def build_bepaid_form_data(payment):
    """
    ...
    order = payment.order
    params = {
        "merchant_id": settings.BEPAID["MERCHANT_ID"],
        "order_id": str(order.public_id),            # public id заказа
        "amount": f"{payment.amount:.2f}",
        "currency": payment.currency or settings.BEPAID.get("CURRENCY", "BYN"),
        "description": f"Order {order.order_number}",
        "return_url": settings.BEPAID["RETURN_URL"],
        "callback_url": settings.BEPAID["CALLBACK_URL"],
    }
    # подпись (пример)
    params["signature"] = sign_bepaid_payload(params)
    return params